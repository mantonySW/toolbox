    ...
    <div className="space-y-8">
      <div className="grid grid-cols-2 gap-6">
        <ProgramSection
          ...
          icon={<Users className="h-6 w-6 text-staccato-graph" />}
        />
        <ProgramSection
          title="Repurchase Programs"
          programs={repurchasePrograms}
          icon={<Target className="h-6 w-6 text-staccato-graph" />}
        />
      </div>
  ...
  return (
    <div className="bg-white shadow-sm rounded-lg p-6">
      <div className="flex items-center space-x-3 mb-6">
        {icon}
        <h2 className="text-xl font-bold text-staccato-primary">{title}</h2>
      </div>
        ...
        {programs.map((program) => (
          <div key={program.name} className="border rounded-lg p-4">
            <div className="flex justify-between items-start mb-3">
              <div>
                <h3 className="font-semibold text-staccato-primary">{program.name}</h3>
                <p className="text-sm text-staccato-gray">{program.description}</p>
              </div>
...
function MetricItem({ label, value }: { label: string; value: number }) {
  return (
    <div className="flex items-center justify-between">
      <span className="text-sm text-staccato-gray">{label}</span>
      <span className="font-semibold text-staccato-primary">{value}%</span>
    </div>
  );
}
...
function PerformanceMetrics() {
  ...
  return (
    <div className="bg-white shadow-sm rounded-lg p-6">
      <h2 className="text-xl font-bold text-staccato-primary mb-6">Overall Performance</h2>
...
function MetricCard({ 
  ...
  return (
    <div className="bg-staccato-alice rounded-lg p-4">
      ...
      <p className="mt-2 text-2xl font-bold text-staccato-primary">
        {value}{isPercentage ? '%' : ''}
      </p>
    </div>
  );
